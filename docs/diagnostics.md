...
Diagnostics also carries a `-- security --` block with one line: which seeding
version the stored key records, read from the record's prefix without decoding
the key. It is stated for every outcome — current, legacy, a version from a newer
build, or no key stored — because "this key was generated by the current path" is
what a device report needs just as much as the warning is. A key from before the
measured barrier also puts one recommending line on the start screen; see
